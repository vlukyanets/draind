...
MprisMonitor::~MprisMonitor() { if (m_bus) sd_bus_unref(m_bus); }
...
bool MprisMonitor::init() {
    ...
    r = sd_bus_match_signal(m_bus, nullptr,
        "org.freedesktop.DBus", "/org/freedesktop/DBus",
        "org.freedesktop.DBus", "NameOwnerChanged",
        trampoline_name_owner, this);
    if (r < 0) {
    ...
    r = sd_bus_match_signal(m_bus, nullptr,
        nullptr, "/org/mpris/MediaPlayer2",
...
void MprisMonitor::scan_existing_players() {
    struct Err { sd_bus_error e = SD_BUS_ERROR_NULL; ~Err() { sd_bus_error_free(&e); } } err;
    struct Msg { sd_bus_message* m = nullptr; ~Msg() { sd_bus_message_unref(m); } } rep;
    ...
    for (int i = 0; names[i]; ++i) {
        ...
        // Resolve unique name
        struct Err2 { sd_bus_error e = SD_BUS_ERROR_NULL; ~Err2() { sd_bus_error_free(&e); } } err2;
        struct Msg2 { sd_bus_message* m = nullptr; ~Msg2() { sd_bus_message_unref(m); } } rep2;
        r = sd_bus_call_method(m_bus,
            "org.freedesktop.DBus", "/org/freedesktop/DBus",
            "org.freedesktop.DBus", "GetNameOwner",
            &err2.e, &rep2.m, "s", name.c_str());
        if (r >= 0) {
...
void MprisMonitor::remove_player(const std::string& well_known) {
    ...
    if (it == m_wk_to_unique.end()) return;
...
void MprisMonitor::query_playback_status(const std::string& unique) {
    ...
    int r = sd_bus_get_property(m_bus,
        unique.c_str(), "/org/mpris/MediaPlayer2",
...
void MprisMonitor::set_player_playing(const std::string& unique, bool playing) {
    ...
    if (any_playing && !m_inhibiting) {
        m_inhibiting = true;
        if (m_on_inhibit) m_on_inhibit(k_inhibit_reason);
    } else if (!any_playing && m_inhibiting) {
        m_inhibiting = false;
        if (m_on_uninhibit) m_on_uninhibit(k_inhibit_reason);
    }
}
...
int MprisMonitor::handle_name_owner_changed(sd_bus_message* m) {
    ...
    if (!is_mpris_name(sname)) return 0;
...
int MprisMonitor::handle_properties_changed(sd_bus_message* m) {
    ...
    if (m_unique_to_wk.find(unique) == m_unique_to_wk.end()) return 0;

    // Read iface
    const char* iface = nullptr;
    sd_bus_message_read(m, "s", &iface);
    if (!iface || strcmp(iface, "org.mpris.MediaPlayer2.Player") != 0) return 0;

    // Scan changed{} for PlaybackStatus
    if (sd_bus_message_enter_container(m, SD_BUS_TYPE_ARRAY, "{sv}") < 0) return 0;
