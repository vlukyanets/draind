...
struct Value {
    ...
    const Array& get_array() const {
        ...
        if (auto* p = std::get_if<Array>(&v)) return *p;
        return empty;
    }
    ...
    bool has(const std::string& key) const {
        if (auto* o = std::get_if<Object>(&v)) return o->count(key) > 0;
        return false;
    }
...
struct Parser {
    ...
    std::string parse_string() {
        ...
        if (p >= end || *p != '"') throw std::runtime_error("json: unterminated string");
        ++p;
        return s;
    }
    ...
    Value parse_object() {
        ...
        while (true) {
            ...
            skip_ws();
            if (peek() == '}') { ++p; break; }
            if (peek() != ',') throw std::runtime_error("json: expected ',' or '}'");
            ++p;
        }
...
inline std::string dump(const Value& val) {
    ...
    if (val.is_int())    return std::to_string(val.get_int());
    if (val.is_double()) {
        char buf[64];
        std::snprintf(buf, sizeof(buf), "%g", std::get<double>(val.v));
        return buf;
    }
    if (val.is_string()) return detail::escape(val.get_string());
    if (val.is_array()) {
        std::string s = "[";
        bool first = true;
