...
bool SessionTracker::init() {
    ...
    r = sd_bus_match_signal(m_bus, nullptr,
        "org.freedesktop.login1", m_seat0_path.c_str(),
...
int SessionTracker::bus_fd() const {
...
void SessionTracker::refresh_active_session() {
    ...
    if (sd_bus_message_enter_container(rep.m, SD_BUS_TYPE_STRUCT, "so") >= 0) {
        ...
        if (sid) m_active_session_id = sid;
