...
Daemon::Daemon(DaemonOptions opts) : m_opts(std::move(opts)) {
    g_log_level = m_opts.log_level;
}

Daemon::~Daemon() {
    if (m_impl && m_impl->epoll_fd >= 0) close(m_impl->epoll_fd);
    delete m_impl;
}
...
static void epoll_del(int epfd, int fd) {
    if (fd >= 0) epoll_ctl(epfd, EPOLL_CTL_DEL, fd, nullptr);
}
...
void Daemon::setup() {
    ...
    im.server.on_connect([](int fd) {
        LOG_DEBUG << "daemon: new connection fd=" << fd;
    });
    im.server.on_line([this](int fd, const std::string& line) {
        on_line(fd, line);
    });
    im.server.on_disconnect([this](int fd) {
        on_disconnect(fd);
    });

    const Profile* p = im.config.active_profile();
    if (p) im.power.apply(*p);

    im.hw_events.on_event([this](HwEvent e) {
        const Profile* p = m_impl->config.active_profile();
        if (!p) return;
        std::string action;
        if      (e == HwEvent::LidClose)    action = p->lid_close_action;
        else if (e == HwEvent::PowerButton)  action = p->power_button_action;
        else if (e == HwEvent::SleepButton)  action = p->sleep_button_action;
        if (!action.empty() && action != "none") {
            if (!p->before_sleep_cmd.empty() &&
                (action == "suspend" || action == "hibernate" || action == "hybrid-sleep"))
...
void Daemon::on_line(int fd, const std::string& line) {
    ...
    if (type == proto::T_HELLO) { on_hello(fd, msg); return; }
    if (type == proto::T_CTL)   { on_ctl(fd, msg);   return; }
    ...
    if      (type == proto::T_IDLE_DIM)   on_idle_dim(fd);
    else if (type == proto::T_IDLE_SLEEP) on_idle_sleep(fd);
    else if (type == proto::T_ACTIVE)     on_active(fd);
    else if (type == proto::T_INHIBIT)    on_inhibit(fd, msg);
    else if (type == proto::T_UNINHIBIT)  on_uninhibit(fd, msg);
    else LOG_WARN << "daemon: unknown message type '" << type << "' fd=" << fd;
}
...
void Daemon::on_inhibit(int fd, const json::Value& msg) {
    ...
    LOG_INFO << "daemon: inhibit from session=" << it->second.session_id
             << " reason=" << reason;
...
void Daemon::on_uninhibit(int fd, const json::Value& msg) {
    ...
    LOG_INFO << "daemon: uninhibit from session=" << it->second.session_id
             << " reason=" << reason;
    m_impl->server.send(fd, proto::encode_ack());
}
...
void Daemon::on_ctl(int fd, const json::Value& msg) {
    ...
    if (cmd == "status") {
        ...
        json::Value r;
        r["ok"] = ok;
        if (!ok) r["error"] = "profile not found: " + name;
        m_impl->server.send(fd, proto::encode_ctl_reply(std::move(r)));
...
bool Daemon::is_active_agent(int fd) const {
    auto it = m_impl->agents.find(fd);
    if (it == m_impl->agents.end()) return false;
...
bool Daemon::has_any_inhibit() const {
    for (const auto& [fd, agent] : m_impl->agents)
        if (!agent.inhibits.empty()) return true;
    return false;
}
...
void Daemon::send_config(int fd) {
    ...
    LOG_DEBUG << "daemon: sent config to fd=" << fd
              << " dim=" << dim << "s sleep=" << sleep << "s";
}
